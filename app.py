...
def get_generated_image():
    if 'username' in session:
        ...
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute('SELECT image_url FROM submissions JOIN users ON submissions.user_id = users.user_id WHERE username = %s ORDER BY submission_datetime DESC LIMIT 1', (username,))
            result = cur.fetchone()
            if result:
                return jsonify({'status': 'success', 'image_url': result[0]})
            else:
